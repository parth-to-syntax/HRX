...
import { performLogout } from '@/redux/slices/userSlice'
...
export default function PageHeader({ 
  ...
  // Auto-compute status if not passed in, so weekends show as Holiday instead of Absent
  const computedStatus = useEmployeeStatus(currentUser?.id)
  const effectiveStatus = userStatus || computedStatus
  ...
  return (
    <div className="flex items-center justify-between gap-4 bg-card border-b h-16 px-4">
      ...
      <div className="flex items-center gap-3">
        ...
        {/* User Status Indicator (non-clickable badge) */}
        {effectiveStatus && (
          <div 
            className={
              `flex items-center gap-2 px-4 py-2 bg-muted/30 rounded-lg border select-none ` +
              (effectiveStatus === 'on-leave' ? 'opacity-90' : '')
            }
          ...
          >
            ...
            ) : effectiveStatus === 'checked-in' || effectiveStatus === 'checked-out' ? (
              <>
