...
export async function listPayslipsForPayrun(req, res) {
  try {
    ...
    const total = countQ.rows[0].c;
    const slipsQ = await pool.query('SELECT * FROM payslips WHERE payrun_id=$1 ORDER BY id ASC LIMIT $2 OFFSET $3', [id, pageSize, offset]);
...
export async function getPayslip(req, res) {
  try {
    ...
    const leave = Number(slip.total_leaves || 0);
    const absent_days = Math.max(0, expected - (present + leave));
    res.json({ payslip: { ...slip, absent_days, expected_working_days: expected }, components: comps.rows, summary: { earnings, deductions } });
  } catch (e) {
    console.error(e);
...
export async function getPayslipPdf(req, res) {
  try {
    const { id } = req.params;
    const ps = await pool.query(
      `SELECT p.*, e.company_id, e.first_name, e.last_name, e.email
         FROM payslips p
         JOIN employees e ON e.id = p.employee_id
        WHERE p.id=$1`,
      [id]
    ...
    doc.text(`Period: ${slip.payrun_id} (Generated: ${new Date(slip.created_at).toISOString().slice(0,10)})`);
